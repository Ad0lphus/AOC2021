...
void print_title() {
    string stuff(25, '-');
    cout << "\033[1;33m";
    cout << stuff <<"Advent of Code - Day 23"  << stuff  << endl;
    cout << "\033[0m";
    cout << string(40, ' ') << endl;
    // run terminal command in c++
    string cmd = "figlet Amphipod -c -f small";
...
int main()
{
    ...
    // get input from ../Input/day23.txt
    ifstream input("../Input/day23.txt");
    for (int i = 0; i < 3; i++)
        getline(input, line);
    ...
    while (!q.empty())
    {
        ...
        if (isDone(s))
        {
            cout << "Puzzle 1: "<< (s.energy)<<endl;
            cout<< "Puzzle 2 : lazy to implement :P" << endl;
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                char c = s.rooms[i][0];
                if (!leave(s, orderMap[c]) && canMove(s, doorway[i], doorway[orderMap[c]], -1))
                {
                    State next = s;
        ...
        for (int i = 0; i < (int)s.hallway.size(); i++)
        {
            ...
            if (c == '.')
                continue;
            if (!leave(s, orderMap[c]) && canMove(s, i, doorway[orderMap[c]], i))
            {
                State next = s;
                next.hallway[i] = '.';
